...
class HistoryProcess extends React.PureComponent {
    constructor(props) {
        super(props);
        this.state={
            stakingList: [],
    ...
    _renderCompleteHeader = (section, index) => {
        console.log(this.state.proceedingActiveSections[0] == '0')
        let idx = index < 9 ? '0' + (index + 1) : (index + 1);
    return (
        <View style={[this.state.proceedingActiveSections[0] == '0' ? styles.AccordionHeaderClose : styles.AccordionHeader, idx == 1 && { borderTopWidth: 1 } ]}>
                <Text>
                    <View>
                        <Text style={[styles.AccordionTitleText, {marginRight: 20}]}>{idx}</Text>
                    </View>
    ...
    _renderContent = section => {
        ...
        let useDay = Math.ceil( useDayGap / ( 1000 * 60 * 60 * 24));
        ...
        if(today < new Date(section.view_pre_pay_date.replace(" ","T"))) {
            return (
                <>
                    ...
                    <AccordionBox keyText={'Deposit period'} valueText={currency(section.stake_period)} divValueText={'Month'} />
                    ...
                    <AccordionBox keyText={'Days of use'} valueText={useDay}  divValueText={'days'}/>
                    <AccordionBox keyText={'Paid cumulative revenue'} valueText={currency(section.unlock_amount != null && section.unlock_amount != undefined && section.unlock_amount != '' ? section.unlock_amount : 0) } divValueText={'DFSD'}/>
                    <AccordionBox keyText={'Expected Payment qty'} valueText={(currency(section.profit_amount))} divValueText={'DFSD'} />
        ...
        //     return (
        //         <>
        //             <AccordionBox keyText={'Deposit amount'} valueText = {section.stake_amount}/>
        //             <AccordionBox keyText={'Annual interest rate'} valueText = {section.stake_profit + '.00%'}/>
        //             <AccordionBox keyText={'Deposit period'} valueText = {section.stake_from_date.split('T')[0] + ' ~ ' + section.pre_pay_date.split('T')[0]}/>
        //             <AccordionBox keyText={'Total return paid'} valueText = {section.stake_amount + section.profit_amount}/>
        //             <AccordionBox borderRadius keyText={'The principal due'} valueText={section.pre_pay_date.split('T')[0]} />
        //         </>
        //     );
        // }
    };
            ...
            <SafeAreaView style={{flex:1, backgroundColor:Color.white}}>
                    ...
                    {/* <View style={styles.tabContainer}>
                        ...
                        <View style={styles.bodyContainer}>
                            ...
                            <>
                            <Text style={styles.proceedingTitle}>Saving in progress</Text>
                            <View>
                            <Accordion
                        ...
                        <>
                        <Text style={styles.completeTitle}>My previous deposit accounts</Text>
                        <View>
                            <Accordion
